	...
	)
{
	if(Do_IQK)
	{
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("AFE IQK mode Success!!!!\n"));
	}
	else
	{
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("AFE Normal mode Success!!!!\n"));
	}
...
{	
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("RestoreRF Success!!!!\n"));
...
{ 
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("ResetNCTL Success!!!!\n"));
}
...
{	
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("============ LOK ============\n"));
	for(Path =0; Path <=3; Path++){
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, 
			("==========S%d LOK ==========\n", Path));
...
{	
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("BandWidth = %d, ExtPA2G = %d\n", *dm->p_band_width, dm->ext_pa));
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("Interface = %d, pBandType = %d\n", dm->support_interface, *dm->p_band_type));
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("CutVersion = %x\n", dm->cut_version));
	...
	)
{
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("=====>PHY_IQCalibrate_8814A_Init\n"));
	for(jj = 0; jj < 2; jj++){
		for(ii = 0; ii < NUM; ii++){
