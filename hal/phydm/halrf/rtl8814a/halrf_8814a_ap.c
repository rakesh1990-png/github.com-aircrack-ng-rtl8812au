	...
	)
{
	...
	{
		PHYDM_DBG(dm,ODM_COMP_TX_PWR_TRACK, ("is 8814 MP chip\n"));
		bit_mask = BIT(19);
	...
	)
{
	...
	PODM_RF_CAL_T	pRFCalibrateInfo = &(dm->RFCalibrateInfo);
		...
		if (Method == MIX_MODE)			
		{
			PHYDM_DBG(dm,ODM_COMP_TX_PWR_TRACK, ("dm->DefaultOfdmIndex=%d, dm->Absolute_OFDMSwingIdx[RFPath]=%d, RF_Path = %d\n",
			...
			)
{
	...
	for(i = 1 ; i < (IQK_MAC_REG_NUM - 1); i++){
		ODM_Write1Byte(dm, MACReg[i], (u1Byte)(MACBackup[i]&(~BIT(3))));
	}
	ODM_Write1Byte(dm, MACReg[i], (u1Byte)(MACBackup[i]&(~BIT(5))));	
			...
			)
{
	...
	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("retry cnt = %d\n", cnt));
			...
			)
{
	...
	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("==>phy_APCalibrate_8814A() delta %d\n", delta));
	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,   ("AP Calibration for %s\n", (is2T ? "2T2R" : "1T1R")));
	if(!is2T)
		pathbound = 1;
	...
	for(path = 0; path < pathbound; path++)
	{
		...
		if(path == RF_PATH_A)
		{
			...
			for(index = 0; index < 11; index ++)			
			{
				...
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_APCalibrate_8814A() offset 0x%x value 0x%x\n", offset, ODM_GetBBReg(dm, offset, MASKDWORD))); 	
		...
		PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_APCalibrate_8814A() offset 0xe70 %x\n", ODM_GetBBReg(dm, rRx_Wait_CCA, MASKDWORD)));		
		...
		PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_APCalibrate_8814A() offset 0x800 %x\n", ODM_GetBBReg(dm, 0x800, MASKDWORD)));				
		...
		for(index = 0; index < APK_BB_REG_NUM; index++)
		{
			...
			if(!dm->RFCalibrateInfo.bAPKThermalMeterIgnore)
			{
				...
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_APCalibrate_8814A() APK index %d tmpReg 0x%x delta_V %d delta_offset %d\n", index, tmpReg, (int)delta_V, (int)delta_offset));
			...
			do
			{
				...
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_APCalibrate_8814A() offset 0xbd8[25:21] %x\n", tmpReg));		
	...
	for(path = 0; path < pathbound; path++)
	{
		...
		if (APK_result[path][1] > 6)
			APK_result[path][1] = 6;
		PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("apk path %d result %d 0x%x \t", path, 1, APK_result[path][1]));					
	}
	...
	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("<==phy_APCalibrate_8814A()\n"));
}
...
{
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,  ("===> PHY_LCCalibrate_8814A\n"));
			...
			)
{
	...
	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("==>phy_DigitalPredistortion_8814A()\n"));

	PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("phy_DigitalPredistortion_8814A for %s %s\n", (is2T ? "2T2R" : "1T1R")));
	...
	while(RetryCount < DP_RETRY_LIMIT && !dm->RFCalibrateInfo.bDPPathAOK)
	{
		...
		if(tmpReg < 0xf0 || tmpReg > 0x105 || tmpReg2 > 0xff )
		{
			...
			ODM_delay_ms(50);			
			RetryCount++;			
			PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("path A DPK RetryCount %d 0xbe0[31:16] %x 0xbe8[31:16] %x\n", RetryCount, tmpReg, tmpReg2));										
		}
		else
		{
			PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("path A DPK Sucess\n"));		
	...
	{	
		...
		for(i=rPdp_AntA; i<=0xb3c; i+=4)
		{
			...
			PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("path A ofsset = 0x%x\n", i));		
		}
	...
	if(is2T)
	{
		...
		while(RetryCount < DP_RETRY_LIMIT && !dm->RFCalibrateInfo.bDPPathBOK)
		{
			...
			if(tmpReg < 0xf0 || tmpReg > 0x105 || tmpReg2 > 0xff)
			{
				...
				ODM_delay_ms(50);			
				RetryCount++;			
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,   ("path B DPK RetryCount %d 0xbf0[31:16] %x, 0xbf8[31:16] %x\n", RetryCount , tmpReg, tmpReg2));														
			}
			else
			{
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("path B DPK Success\n"));						
		...
		if(dm->RFCalibrateInfo.bDPPathBOK)
		{
			...
			{
				ODM_SetBBReg(dm, i, MASKDWORD, 0x40004000);	
				PHYDM_DBG(dm,ODM_COMP_CALIBRATION,  ("path B ofsset = 0x%x\n", i));
			}
